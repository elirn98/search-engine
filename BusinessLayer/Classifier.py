...
class Train_data:
    def __init__(self):
        self.sheet = pd.read_excel(
            r'E:/pythonProjects/IRsystem/IR-project-data-phase-3-100k/train/train.xlsx')
...
class Classifier:
    ...
    def KNN(self):
        classes = {"science": [], "culture-art": [], "politics": [], "economy": [], "social": [], "international": [],
                   "sport": [], "multimedia": []}
    ...
    def NB(self):
        classes = {"science": [], "culture-art": [], "politics": [], "economy": [], "social": [], "international": [],
                   "sport": [], "multimedia": []}
        print("for size : " + str(len(self.docVectorList)))
        class_tf, nci = self.get_classes_tf()
        for key in classes.keys():
            classes[key].append([])
        for j in range(len(self.docVectorList)):
            print("j: " + str(j))
    ...
    def get_classes_tf(self):
        class_tf = {"science": 0, "culture-art": 0, "politics": 0, "economy": 0, "social": 0, "international": 0,
                    "sport": 0, "multimedia": 0}
        nci = {"science": 0, "culture-art": 0, "politics": 0, "economy": 0, "social": 0, "international": 0,
               "sport": 0, "multimedia": 0}
        for t in range(len(self.trainVectorList)):
    ...
    def determine_category(self, docVector, class_tf, nci):
        c_score = {"science": 0, "culture-art": 0, "politics": 0, "economy": 0, "social": 0, "international": 0,
                   "sport": 0, "multimedia": 0}
        for cat in c_score.keys():
            c_score[cat] += math.log10(nci[cat] / 1000)
            for tf in docVector.values():
                c_score[cat] += math.log10((tf + 1) / class_tf[cat])
